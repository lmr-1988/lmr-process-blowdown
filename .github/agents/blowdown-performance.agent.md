...
7. **Status per segment.** Apply the user-supplied acceptance criteria → **PASS / REVIEW / INCONCLUSIVE / ALREADY DEPRESSURISED**. Status definitions: **PASS** = measured curve stays within acceptance criteria throughout; **REVIEW** = measured curve exceeds acceptance criteria (requires engineering review); **INCONCLUSIVE** = data is missing, the event is ambiguous, or the segment was not simulated; **ALREADY DEPRESSURISED** = the measured starting pressure at `t = 0` is below **3 barg** — do not issue PASS or REVIEW, and do not assess against the curve-deviation criterion, since the segment was already depressurised before the event and this is not a judgement of valve/blowdown performance. Show this status with a light-green badge (`#b7e4c7` background, `#1b4332` text) in reports.
...
- **DO NOT** apply the normal PASS/REVIEW curve-deviation criterion to a segment whose measured starting pressure is below **3 barg** — classify it as `ALREADY DEPRESSURISED` instead (see Approach, step 7).
...
- **One separate chart per segment** — never combine segments on a single chart. Each evaluated segment gets its own report section so its axes auto-scale to that segment's pressure range. For each evaluated segment, include:
  - A **PASS / REVIEW / INCONCLUSIVE / ALREADY DEPRESSURISED** status badge (light-green `#b7e4c7`/`#1b4332` for `ALREADY DEPRESSURISED`).
